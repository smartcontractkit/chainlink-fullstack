...
skip.if(!developmentChains.includes(network.name)).
  describe('RandomSVG Unit Tests', () => {
    ...
    beforeEach(async () => {
      ...
      rsNFT = await ethers.getContractAt("RandomSVG", RandomSVG.address) as unknown as RandomSVG;
    ...
    it('should return the correct URI', async () => {
      ...
      const [, requestId, tokenId] = receipt.events && receipt.events[3].topics || [];
      const fakeRandomNumber = 77777;

      const transactionResponse = await vrfCoordinator.callBackWithRandomness(requestId, fakeRandomNumber, rsNFT.address);
